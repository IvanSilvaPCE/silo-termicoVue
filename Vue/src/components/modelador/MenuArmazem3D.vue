...
export default {
  ...
  methods: {
    ...
    buildParedesTriangularesPontudas(paredeMaterial, espessuraParede, alturaTelhado, config) {
      ...
      // Geometria triangular para as paredes frontais
      const shape = new THREE.Shape();
      
      if (pontas_redondas) {
        // Paredes com pontas arredondadas
        const largura = this.profundidadeArmazem;
        const altura = this.alturaArmazem;
        const alturaPico = alturaTelhado - altura;

        shape.moveTo(-largura/2, 0);
        shape.lineTo(-largura/2, altura);
        
        // Ponta arredondada esquerda
        shape.quadraticCurveTo(-largura/2 + raio_pontas, altura + alturaPico/2, 0, altura + alturaPico);
        
        // Ponta arredondada direita
        shape.quadraticCurveTo(largura/2 - raio_pontas, altura + alturaPico/2, largura/2, altura);
        
        shape.lineTo(largura/2, 0);
        shape.lineTo(-largura/2, 0);
      } else {
        // Paredes triangulares simples
        const largura = this.profundidadeArmazem;
        const altura = this.alturaArmazem;
        const alturaPico = alturaTelhado - altura;
        ...
        shape.moveTo(-largura/2, 0);
        shape.lineTo(-largura/2, altura);
        shape.lineTo(0, altura + alturaPico);
        shape.lineTo(largura/2, altura);
        shape.lineTo(largura/2, 0);
        shape.lineTo(-largura/2, 0);
      }
      ...
      // Parede frontal
      const paredeFrontal = new THREE.Mesh(paredeGeometry, paredeMaterial);
      paredeFrontal.position.set(-this.larguraArmazem / 2, 0, 0);
      paredeFrontal.rotation.y = Math.PI / 2;
      paredeFrontal.castShadow = true;
      paredeFrontal.userData = { tipo: 'estrutura_armazem' };
      this.scene.add(paredeFrontal);

      // Parede traseira
      const paredeTraseira = new THREE.Mesh(paredeGeometry, paredeMaterial);
      paredeTraseira.position.set(this.larguraArmazem / 2, 0, 0);
      paredeTraseira.rotation.y = -Math.PI / 2;
      paredeTraseira.castShadow = true;
      paredeTraseira.userData = { tipo: 'estrutura_armazem' };
      this.scene.add(paredeTraseira);
    },

    buildParedesArredondadas(paredeMaterial, espessuraParede, alturaTelhado, curvatura_topo) {
      // Criar paredes com topo curvado
      const segmentos = 16;
      const altura = this.alturaArmazem;
      const alturaCurvatura = (alturaTelhado - altura);
      const raio = curvatura_topo / 100 * this.profundidadeArmazem / 2;
      ...
      for (let i = 0; i <= segmentos; i++) {
        const angle = (i / segmentos) * Math.PI;
        const x = Math.cos(angle) * raio;
        const y = altura + Math.sin(angle) * raio;
        shape.lineTo(-largura/2 + (i / segmentos) * largura, y);
      }
    ...
    buildParedesArco(paredeMaterial, espessuraParede, alturaTelhado, curvatura_topo) {
      // Criar paredes em formato de arco completo
      const segmentos = 20;
      const altura = this.alturaArmazem;
      const raioArco = curvatura_topo / 100 * this.profundidadeArmazem / 2;
      ...
      for (let i = 0; i <= segmentos; i++) {
        const angle = (i / segmentos) * Math.PI;
        const x = -largura/2 + (i / segmentos) * largura;
        const y = altura + Math.sin(angle) * raioArco;
        shape.lineTo(x, y);
      }
    ...
    buildTelhadoComConfiguracao(corTelhado, config) {
      const tipo_telhado = config.tipo_telhado || 1;
      const curvatura_topo = config.curvatura_topo || 30;
      const pontas_redondas = config.pontas_redondas || false;
      ...
      if (tipo_telhado === 1) {
        // Telhado pontudo/triangular
        this.buildTelhadoPontudo(telhadoMaterial, alturaTelhado, pontas_redondas, config);
      } else if (tipo_telhado === 2) {
        // Telhado arredondado/curvado
        this.buildTelhadoArredondado(telhadoMaterial, alturaTelhado, curvatura_topo);
      } else if (tipo_telhado === 3) {
        // Telhado em arco
        this.buildTelhadoArco(telhadoMaterial, alturaTelhado, curvatura_topo);
      }
    },

    buildTelhadoPontudo(telhadoMaterial, alturaTelhado, pontas_redondas, config) {
      if (pontas_redondas) {
        // Telhado com pontas arredondadas
        const raio_pontas = (config.raio_pontas || 15) / 100;
        const segmentos = 16;
        
        const geometry = new THREE.CylinderGeometry(
          raio_pontas, raio_pontas, this.larguraArmazem, segmentos, 1, false, 0, Math.PI
        );
        
        const telhado = new THREE.Mesh(geometry, telhadoMaterial);
        telhado.position.set(0, alturaTelhado, 0);
        telhado.rotation.z = Math.PI / 2;
        telhado.castShadow = true;
        telhado.userData = { tipo: 'estrutura_armazem' };
        this.scene.add(telhado);
      } else {
        // Telhado triangular simples
        const telhadoGeometry = new THREE.ConeGeometry(
          this.profundidadeArmazem / 2, 
          alturaTelhado - this.alturaArmazem, 
          4, 1, false, 0, Math.PI
        );
        
        const telhado = new THREE.Mesh(telhadoGeometry, telhadoMaterial);
        telhado.position.set(0, alturaTelhado - (alturaTelhado - this.alturaArmazem) / 2, 0);
    ...
    buildTelhadoArredondado(telhadoMaterial, alturaTelhado, curvatura_topo) {
      // Telhado curvado
      const segmentos = 20;
      const raio = curvatura_topo / 100 * this.profundidadeArmazem / 2;
      
      const telhadoGeometry = new THREE.CylinderGeometry(
        raio, raio, this.larguraArmazem, segmentos, 1, false, 0, Math.PI
      );
      
      const telhado = new THREE.Mesh(telhadoGeometry, telhadoMaterial);
      telhado.position.set(0, this.alturaArmazem + raio, 0);
      telhado.rotation.z = Math.PI / 2;
      telhado.castShadow = true;
      telhado.userData = { tipo: 'estrutura_armazem' };
      this.scene.add(telhado);
    ...
    buildTelhadoArco(telhadoMaterial, alturaTelhado, curvatura_topo) {
      // Telhado em arco completo
      const segmentos = 24;
      const raioArco = curvatura_topo / 100 * this.profundidadeArmazem / 2;
      
      const telhadoGeometry = new THREE.CylinderGeometry(
        raioArco, raioArco, this.larguraArmazem, segmentos, 1, false, 0, Math.PI
      );
      
      const telhado = new THREE.Mesh(telhadoGeometry, telhadoMaterial);
      telhado.position.set(0, this.alturaArmazem + raioArco, 0);
      telhado.rotation.z = Math.PI / 2;
      telhado.castShadow = true;
      telhado.userData = { tipo: 'estrutura_armazem' };
      this.scene.add(telhado);
    ...
    buildFundoReto(corBase, config) {
      const altura_fundo_reto = (config.altura_fundo_reto || 10) / 100;
      ...
      fundo.position.set(0, 0.15 + altura_fundo_reto/2, 0);
    ...
    buildFundoFunilV(corBase, config) {
      const altura_funil_v = (config.altura_funil_v || 18) / 100;
      const largura_abertura_v = (config.largura_abertura_v || 20) / 100;
      const posicao_ponta_v = config.posicao_ponta_v || 0;
      ...
      // Funil em V
      const funilGeometry = new THREE.ConeGeometry(largura_abertura_v, altura_funil_v, 8);
      const funil = new THREE.Mesh(funilGeometry, baseMaterial);
      funil.position.set(posicao_ponta_v, 0.25 - altura_funil_v/2, 0);
      funil.castShadow = true;
      funil.userData = { tipo: 'estrutura_armazem' };
      this.scene.add(funil);
    },

    buildFundoDuploV(corBase, config) {
      const altura_duplo_v = (config.altura_duplo_v || 22) / 100;
      const posicao_v_esquerdo = config.posicao_v_esquerdo || -1;
      const posicao_v_direito = config.posicao_v_direito || 1;
      const largura_abertura_duplo_v = (config.largura_abertura_duplo_v || 2) / 100;
      const altura_plataforma_duplo_v = (config.altura_plataforma_duplo_v || 0.3) / 100;
      const largura_plataforma_duplo_v = (config.largura_plataforma_duplo_v || 10) / 100;
    ...
    buildAeradores() {
      if (!this.dados?.configuracao?.layout_topo?.aeradores) return;

      const aeradores = this.dados.configuracao.layout_topo.aeradores;
      const aer = this.dados.AER ? this.dados.AER.split(',').map(status => parseInt(status)) : [];
      ...
      Object.entries(aeradores).forEach(([aeradorId, posicao], index) => {
        const [posXSVG, posYSVG] = posicao;
        const status = aer[index] || 0;

        // Converter posições SVG para 3D
        const posX = -this.larguraArmazem / 2 + (posXSVG / tamanhoSVG[0]) * this.larguraArmazem;
        const posZ = -this.profundidadeArmazem / 2 + ((posYSVG - 50) / 250) * (this.profundidadeArmazem - 2) + 1;

        this.buildAerador(posX, posZ, parseInt(aeradorId), status);
      });
    },

    buildAerador(posX, posZ, aeradorId, status) {
      // Status: 0=Desligado, 1=Ligado Manual, 2=Ligado Auto, 3=Funcionando
      const cores = {
        0: 0x666666, // Cinza - Desligado
        1: 0xffff00, // Amarelo - Manual
        2: 0x00ff00, // Verde - Auto
        3: 0x00ffff  // Azul - Funcionando
      };

      const aeradorGroup = new THREE.Group();
      aeradorGroup.position.set(posX, 0, posZ);

      // Base da tubulação
      const tuboGeometry = new THREE.CylinderGeometry(0.15, 0.18, 0.6, 16);
      const tuboMaterial = new THREE.MeshStandardMaterial({
        color: 0x444444,
        metalness: 0.7,
        roughness: 0.3
      });
      const tubo = new THREE.Mesh(tuboGeometry, tuboMaterial);
      tubo.position.set(0, 0.3, 0);
      tubo.castShadow = true;
      aeradorGroup.add(tubo);

      // Hélice do aerador
      const heliceGroup = new THREE.Group();
      
      // Centro da hélice
      const centroGeometry = new THREE.CylinderGeometry(0.03, 0.03, 0.1, 8);
      ...
      heliceGroup.add(centro);

      // Pás da hélice
      const numPas = 6;
      for (let i = 0; i < numPas; i++) {
        const angle = (i / numPas) * Math.PI * 2;
        const paGeometry = new THREE.BoxGeometry(0.4, 0.02, 0.08);
        ...
        pa.position.set(Math.cos(angle) * 0.2, 0, Math.sin(angle) * 0.2);
        pa.rotation.y = angle + Math.PI / 2;
        pa.rotation.x = Math.PI / 8; // Inclinação das pás
        pa.castShadow = true;
        heliceGroup.add(pa);
      }

      heliceGroup.position.set(0, 1.0, 0);
      this.aeradorHélices.push(heliceGroup);
      aeradorGroup.add(heliceGroup);

      // Motor do aerador
      const motorGeometry = new THREE.CylinderGeometry(0.12, 0.12, 0.25, 12);
